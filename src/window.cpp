...
SequenceController::SequenceController() 
    : chip_("gpiochip0"), 
      line1_(chip_.get_line(27)),
      line2_(chip_.get_line(22)) 
{
    line1_.request({"switch1", gpiod::line_request::DIRECTION_OUTPUT, 0}, 1);
...
Window::Window(QWidget *parent) : QWidget(parent) {
    ...
    try {
        ...
        gpioController = std::make_unique<SequenceController>();
...
Window::~Window() {
    ...
    delete servoThread;
    delete servoDriver;
    if (gpioController) gpioController->stop();
}
...
void Window::calculateServoAngles(const cv::Point &target, const cv::Size &frameSize) {
    ...
    newPan = qBound(20.0f, newPan, 160.0f);
    newTilt = qBound(90.0f, newTilt, 135.0f);
...
void Window::scanningMovement() {
    ...
    if(m_currentPan >= 180 || m_currentPan <= 0) {
        m_scanDirection *= -1;
        m_currentTilt = qBound(90.0f, m_currentTilt + 5.0f, 135.0f);
        if(m_currentTilt >= 135) m_currentTilt = 90;
    }
...
void Window::updateImage(const cv::Mat &mat) {
    ...
    if (trackingEnabled) {
        if(m_hasTarget) {
            ...
            float dx = (m_targetCenter.x - frameCenter.x) / (float)frameCenter.x;
            float dy = (m_targetCenter.y - frameCenter.y) / (float)frameCenter.y;

            if (std::abs(dx) < CENTER_THRESHOLD && std::abs(dy) < CENTER_THRESHOLD && !gpioTriggered) {
                gpioController->start_sequence();
                gpioTriggered = true;
                thermo->setValue(255);
            } else if (std::abs(dx) >= CENTER_THRESHOLD || std::abs(dy) >= CENTER_THRESHOLD) {
                gpioTriggered = false;
                thermo->setValue(0);
            }
